...
class MicropyGPS(object):
    ...
    def date_string(self, formatting='s_mdy', century='20'):
        ...
        # Timezone for is already manage in the class, this snippet is to manage the timezone for the date
        # Retreive the original UTC
        # Apply TimeZone to the UTC
        # Return date string with the timezone compute
        #
        utc_tuple = (int(century)*100+self.date[2], self.date[1], self.date[0], (self.timestamp[0] - self.local_offset) % 24, self.timestamp[1], int(self.timestamp[2]), 0, 0)
        utc_timestamp = utime.mktime(utc_tuple)
        timezone_offset = 3600 * self.local_offset
        local_timestamp = utc_timestamp + timezone_offset
        local_datetime = utime.localtime(local_timestamp)
        timezone_date = [local_datetime[2], local_datetime[1], local_datetime[0]]
        timezone_date[2] = timezone_date[2]-int(century)*100
        
        # Long Format Januray 1st, 2014
        if formatting == 'long':
            # Retrieve Month string from private set
            month = self.__MONTHS[timezone_date[1] - 1]

            # Determine Date Suffix
            if timezone_date[0] in (1, 21, 31):
                suffix = 'st'
            elif timezone_date[0] in (2, 22):
                suffix = 'nd'
            elif timezone_date[0] == (3, 23):
                suffix = 'rd'
            else:
                suffix = 'th'

            day = str(timezone_date[0]) + suffix  # Create Day String

            year = century + str(timezone_date[2])  # Create Year String

            date_string = month + ' ' + day + ', ' + year  # Put it all together

        else:
            # Add leading zeros to day string if necessary
            if timezone_date[0] < 10:
                day = '0' + str(timezone_date[0])
            else:
                day = str(timezone_date[0])

            # Add leading zeros to month string if necessary
            if timezone_date[1] < 10:
                month = '0' + str(timezone_date[1])
            else:
                month = str(timezone_date[1])

            # Add leading zeros to year string if necessary
            if timezone_date[2] < 10:
                year = '0' + str(timezone_date[2])
            else:
                year = str(timezone_date[2])
